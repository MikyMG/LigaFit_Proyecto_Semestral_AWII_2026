...
func ObtenerSeguimientosPorDeportista(deportistaID int) []models.SeguimientoFisico {
	...
	for _, seguimiento := range seguimientoRepo.ListarSeguimientos() {
...
func ActualizarSeguimiento(id int, datos models.SeguimientoFisico) (models.SeguimientoFisico, error) {
	seguimientoAnterior, encontrado := seguimientoRepo.BuscarSeguimientoPorID(id)
	if !encontrado {
		return models.SeguimientoFisico{}, errors.New("seguimiento no encontrado")
	}

	if datos.DeportistaID <= 0 {
		return seguimientoAnterior, errors.New("el deportista_id es obligatorio")
	}

	if datos.EntrenadorID <= 0 {
		return seguimientoAnterior, errors.New("el entrenador_id es obligatorio")
	}

	if datos.Peso <= 0 {
		return seguimientoAnterior, errors.New("el peso debe ser mayor a 0")
	}

	if datos.Altura <= 0 {
		return seguimientoAnterior, errors.New("la altura debe ser mayor a 0")
	}

	if datos.Altura < 0.5 || datos.Altura > 2.5 {
		return seguimientoAnterior, errors.New("la altura debe estar entre 0.5 y 2.5 metros")
	}
	...
	datos.CreatedAt = seguimientoAnterior.CreatedAt
	datos.UpdatedAt = time.Now()

	if datos.FechaRegistro == "" {
		datos.FechaRegistro = seguimientoAnterior.FechaRegistro
	}
